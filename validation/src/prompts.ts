...
Return one raw JSON object matching the supplied schema, with no Markdown fence, deliberation, or commentary. Keep reason under 300 characters and the complete diff under the supplied maxDiffLines. For PATCH, emit a complete repository-rooted standard unified diff that applies exactly to the supplied base files. Every changed file must have "diff --git a/path b/path", "--- a/path", and "+++ b/path" headers followed by valid @@ hunk ranges whose line counts match the hunk body. Do not emit no-op hunks or lines that differ only by trailing whitespace. addressedRequirementIds must contain only findings fully implemented by the replacement diff. If no compliant replacement exists, return BLOCKED_MISSING_VALUE with an empty diff and an empty addressedRequirementIds array. If unchanged base code already satisfies the finding, return BLOCKED_AUDIT_CONFLICT with an empty diff and no addressed IDs. All path, hash, ownership, size, git-apply, test, re-audit, regression, conflict, and publication guards still apply.`;
