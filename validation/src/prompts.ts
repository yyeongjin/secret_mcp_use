...
Every added or removed line must directly implement one of the supplied findings. Do not substitute a different fact from the same Section, even when that fact is grounded in the contract. If the finding names logo bounds, for example, a z-index, header height, or unrelated navigation value is outside scope. A PATCH response must contain at least one actual '+' line and may contain '-' lines only when replacing the exact implementation identified by that finding.
