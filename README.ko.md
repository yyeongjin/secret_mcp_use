...
검증된 코드 PR은 본문 최상단에 requirement-level finding 전체와 실제 unified diff를 싣고, 변경 줄 수, 실제 request ID, guard 결과와 실행 artifact를 이어서 표시합니다. 차단, UNKNOWN, dependency 대기 또는 실패 결과는 빈 PR이나 report-only PR로 만들지 않습니다. 대신 같은 finding 원문을 Section Check와 멱등적인 피드백 Issue에 게시합니다. 피드백 Issue는 fingerprint가 달라져도 같은 Section 작업으로 갱신되며 PASS 또는 검증된 코드 PR이 생기면 자동으로 닫힙니다.
