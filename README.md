...
| `PIPELINE_CREATE_PRS` | `true` | Publishes idempotent draft PRs for complete verified Section diffs; DAG-waiting omissions remain queued instead of becoming issues |
...
Every verified code PR leads with every Requirement ID assigned to that Section correction, followed by changed-line counts, concrete request IDs, guard results, and the run artifact. The patch model must implement the complete supplied finding set; partial Section PRs are rejected and replaced by another independent candidate. A separate stateless before/after re-audit verifies every claim. A grounded `PATCH_REQUIRED` result that is waiting for a non-PASS dependency remains in the persisted DAG queue and does not create a feedback issue. Merging the prerequisite PR triggers a fresh fingerprint calculation, reuses unchanged PASS attestations, and automatically schedules the newly ready Section. Feedback issues are reserved for a true contract/evidence gap or an exhausted provider/verification failure that cannot produce a grounded code diff.

An open automation PR is never closed and its branch is never deleted merely because `main` changed. The pipeline marks it stale, performs a fresh isolated audit against the current base, and preserves the PR number and review history. When a replacement diff passes every guard, only the bot-owned automation branch is updated with `--force-with-lease`, and the same PR title, body, manifest, and diff are refreshed. If the new result is PASS, blocked, or cannot be patched safely, the PR remains open for a human decision and the current verbal feedback is published; the automation does not close it on the user's behalf.

When an isolated patch candidate claims that the current base already satisfies an audit finding, lacks a value, or cannot fit the patch scope, that single candidate is not allowed to cancel a grounded omission. The orchestrator requests another independently seeded candidate for the same Section until `PIPELINE_PATCH_ATTEMPTS` is exhausted. Malformed responses, invalid diffs, incomplete Requirement ID coverage, failed checks, and failed re-audits use the same bounded replacement-candidate budget. A terminal blocked result is published only after every allowed independent candidate has failed.

One `PATCH_REQUIRED` Section may receive up to `PIPELINE_PATCH_ATTEMPTS` complete patch candidates. Every candidate is a separate NVIDIA request with a distinct deterministic seed, the same isolated Section contract, and the unchanged base files. The request contains only implementation files named by that Section's findings, together with a numbered physical-line view so a one-line source rule remains one exact diff line. A candidate must implement every supplied Requirement ID; partial coverage, invalid JSON or schema, repairable diff mechanics, blocked candidate judgments, failed tests, a non-PASS patched-Section re-audit, or a failed affected-PASS regression audit discards that candidate and starts the next bounded candidate. A retry receives a bounded summary of only its own rejected output plus its failure diagnostic; it never receives another Section's contract, response, or diff. Immutable-path writes, cross-owner writes, unsafe paths or file operations, excessive scope, write-set conflicts, and publication conflicts never weaken a guard. Dependency and write-lock conflicts remain queued for a later main revision; other exhausted failures are reported explicitly. Every attempt is stored under `patches/SXX/attempt-N/`. PASS Sections never create patch requests or PRs, and true evidence gaps can create feedback only after isolated audit retries are exhausted.
