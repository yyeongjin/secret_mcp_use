...
| `PIPELINE_CREATE_PRS` | `true` | Publishes idempotent draft PRs for verified diffs and feedback issues for actionable results without a safe diff |
...
Every verified code PR leads with only the Requirement IDs that its actual unified diff fully corrects, followed by changed-line counts, concrete request IDs, guard results, and the run artifact. The patch model must declare those IDs, and a separate stateless before/after re-audit must verify every claim. Findings not implemented by a partial PR remain explicitly listed as unresolved and are published in the Section Check and an idempotent feedback issue. Blocked, unknown, dependency-waiting, or failed results also use verbal feedback instead of empty/report-only PRs. A feedback issue closes only after PASS or after every finding is covered by a verified code PR.
...
When the isolated patch request finds that the current base code already satisfies an audit finding, it returns `BLOCKED_AUDIT_CONFLICT` with the exact existing-code explanation. Missing values, excessive patch scope, and audit conflicts are terminal feedback results, not five repeated patch attempts. Only malformed responses, invalid diffs, failed checks, or failed re-audits consume the bounded replacement-candidate budget.
