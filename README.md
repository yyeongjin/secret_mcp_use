...
Every verified code PR leads with the complete requirement-level findings and the actual unified diff, followed by changed-line counts, concrete request IDs, guard results, and the run artifact. A blocked, unknown, dependency-waiting, or failed result never becomes an empty/report-only PR: its complete verbal feedback is published in the Section Check and an idempotent feedback issue. The issue is updated across fingerprints and closes automatically after a PASS or verified code PR.
