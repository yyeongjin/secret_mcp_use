...
Every verified code child PR leads with the Requirement ID assigned to that correction, followed by changed-line counts, concrete request IDs, guard results, and the run artifact. `S01` through `S19` remain top-level aggregation Sections. Stage 1 creates one readable Markdown report child PR for every non-PASS Section, merges report children deepest-first in groups of at most five, and leaves one report representative draft PR plus one representative Issue per work. Each report prints the exact Specification source text and exact gap finding text; normalized output JSON, leaf-record JSON, and `manifest.json` are forbidden from published reports. `DOCUMENT_GAPS.md` embeds each plain-text Section report byte-for-byte and `MANIFEST.md` provides a readable hash table. Oversized Issue payloads are split into ordered comments that reconstruct the exact file. Legacy Section Issue bodies are copied verbatim into the representative Issue before the old Issues are linked and closed. Stage 2 creates dynamic code children such as `S09-1`, `S09-2`, and `S09-3`; each child owns exactly one Requirement. Every such Requirement receives two independent current-source preflights, with a third tie-break when the first two disagree, so patching or reclassification requires a 2-of-3 decision. The resulting patch is independently guarded and re-audited. Code children are merged deepest-first in groups of at most five, leaving one `S09`-style draft PR as the human review boundary. The normative pipeline contract is `IDEA_VALIDATION_AND_PR_PIPELINE.ko.md`.
