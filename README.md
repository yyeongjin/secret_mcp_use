...
| `PIPELINE_CREATE_PRS` | `true` | Publishes idempotent draft PRs for complete verified Section diffs; all non-PR results stay in Checks and run artifacts, never GitHub Issues |
...
Every verified code PR leads with every Requirement ID assigned to that Section correction, followed by changed-line counts, concrete request IDs, guard results, and the run artifact. The patch model must implement the complete supplied finding set; partial Section PRs are rejected and replaced by another independent candidate. A separate stateless before/after re-audit verifies every claim. A grounded `PATCH_REQUIRED` result that is waiting for a non-PASS dependency remains in the persisted DAG queue. Merging the prerequisite PR triggers a fresh fingerprint calculation, reuses unchanged PASS attestations, and automatically schedules the newly ready Section. The pipeline never creates GitHub Issues. Results without a complete verified code diff remain only in the Section Check and immutable run artifact.
