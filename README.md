...
Every verified code PR leads with every Requirement ID assigned to that child correction, followed by changed-line counts, concrete request IDs, guard results, and the run artifact. `S01` through `S19` remain the top-level audit Sections. When one Section is too large for a single bounded patch response, the patch stage creates dynamic child nodes such as `S09-1`, `S09-2`, and `S09-3`. Each child receives only the Requirement IDs still unresolved at that point, runs as a separate NVIDIA request, and is independently guarded and re-audited. The first child targets the workflow base (`S09-1 -> main`), and each later child targets the preceding child branch (`S09-2 -> S09-1`, `S09-3 -> S09-2`). This lets reviewers merge the deepest child upward without combining unrelated child diffs into one oversized PR. A grounded `PATCH_REQUIRED` result that is waiting for a non-PASS dependency remains in the persisted DAG queue. The pipeline never creates GitHub Issues.
...
One `PATCH_REQUIRED` Section may create as many child nodes as are required to cover its finite Requirement ID set; `19` is not a patch-call ceiling. Each child gets up to `PIPELINE_PATCH_ATTEMPTS` replacement candidates, and every candidate is a separate NVIDIA request with a distinct request ID. The request contains only the still-unresolved findings and the implementation files they name. A candidate must make new progress by fully implementing at least one supplied Requirement ID. Invalid JSON or schema, no progress, blocked candidate judgments, failed tests, a non-PASS child re-audit, or a failed affected-PASS regression audit discards that candidate and retries only the same child. A verified partial candidate becomes its own stacked child PR; the next child is rebuilt from that published parent commit and receives only the remaining IDs. Retries receive only their own bounded rejection summary, never another Section's contract, response, or diff. Immutable-path writes, cross-owner writes, unsafe paths or file operations, excessive scope, cross-Section write conflicts, and publication conflicts never weaken a guard. Attempts are stored under `patches/SXX/SXX-N/attempt-M/`. PASS Sections never create patch requests or PRs.
